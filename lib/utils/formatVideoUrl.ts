...
const unmutedTrailerParams = "controls=0&modestbranding=1&rel=0&showinfo=0&iv_load_policy=3&disablekb=1&fs=0&enablejsapi=1&mute=0";
...
export function formatVideoEmbedUrl(url: string | undefined): string {
  ...
  if (formatted.includes("youtube.com/watch")) {
    ...
    if (videoId) return `https://www.youtube-nocookie.com/embed/${videoId}?${unmutedTrailerParams}`;
  }
  ...
  if (formatted.includes("vimeo.com/") && !formatted.includes("player.vimeo.com")) {
    ...
    if (videoId) return `https://player.vimeo.com/video/${videoId}?controls=0&title=0&byline=0&portrait=0&muted=0`;
  }
