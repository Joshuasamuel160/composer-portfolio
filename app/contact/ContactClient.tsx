...
import { ContactInfoData } from "@/lib/mockData";
import { ScrollAnimation } from "@/components/ScrollAnimation";
import { Copy, Check, Mail, ArrowUpRight, MapPin, Building2 } from "lucide-react";
...
export const ContactClient: React.FC<ContactClientProps> = ({ contactInfo }) => {
  ...
  const email = contactInfo.email || "contact@joshuasamuel.com";
  ...
  return (
    <div className="py-20 px-6 max-w-5xl mx-auto space-y-16">
      ...
      {/* Representation & Locations Info */}
      {(contactInfo.representation || contactInfo.locations) && (
        <ScrollAnimation delay={0.15}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {contactInfo.representation && (
              <div className="cinematic-card rounded-2xl p-6 border border-white/5 space-y-2">
                <div className="flex items-center gap-2 text-xs font-mono text-amber-400 uppercase tracking-widest">
                  <Building2 size={16} /> REPRESENTATION & INQUIRIES
                </div>
                <p className="text-sm text-zinc-300 font-light whitespace-pre-line leading-relaxed">
                  {contactInfo.representation}
                </p>
              </div>
            )}
            ...
            {contactInfo.locations && (
              <div className="cinematic-card rounded-2xl p-6 border border-white/5 space-y-2">
                ...
                <p className="text-sm text-zinc-300 font-light uppercase tracking-wider font-mono pt-1">
                  {contactInfo.locations}
                </p>
              </div>
            )}
